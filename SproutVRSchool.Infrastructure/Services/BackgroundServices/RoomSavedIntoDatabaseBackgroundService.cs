...
public class RoomSavedIntoDatabaseBackgroundService : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            RedisValue vrLearningSessionId = await _database.SetPopAsync(AppCts.Redis.NAMESPACE_VR_LEARNING_SESSIONS_INACTIVE);
            ...
            try
            {
                ...
                await SaveSessionToDatabaseAsync(sessionData, stoppingToken);

                // 5. Delete the id key fron the vr_learning_session group
    ...
    private async Task SaveSessionToDatabaseAsync(ModelVRLearningSession? modelVRLearningSession, CancellationToken stoppingToken)
    {
        ...
        try
        {
            ...
            {
                VRDevice vrDevice = await uow.Repository<VRDevice>().GetEntityBySpec(new VRDevicesSpecification(serialNumber: modelVrDevice.VrDeviceSerialNumber));
